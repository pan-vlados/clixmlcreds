...
from src.clixmlcreds import CredentialManager, Credential
...
class TestCredentials(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.credential: Credential = CredentialManager.read(cred_name=self.cred_name)
    ...
    def test_read(self):
        credential: Credential = CredentialManager.read(cred_name=self.cred_name)
        self.assertEqual(credential.username, self.username)

    def test_get_password(self):
        password = self.credential.get_password()
        decrypted_password, _ = CredentialManager.decrypt(password=self.credential.password)
        self.assertEqual(password, decrypted_password)

    def test_encrypt_decrypt(self):
        password = self.credential.get_password()
        encrypted_password = CredentialManager.encrypt(password=password)
        decrypted_password, _ = CredentialManager.decrypt(password=encrypted_password)
        self.assertEqual(password, decrypted_password)
